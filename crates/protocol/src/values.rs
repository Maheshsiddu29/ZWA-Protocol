...
///
/// This is private witness material: knowledge of it reproduces the subject
/// commitment and therefore lets its holder bind any receiver into a recipient
/// commitment. It must never be logged or published.
///
/// Unlike every other value in this module, `SubjectSecret` is written out by
/// hand rather than generated by the newtype macro, so that it does **not**
/// print its value. Its [`Debug`] output is redacted and it implements no
/// [`Display`](core::fmt::Display), which keeps it out of log lines, `dbg!`
/// output, panic messages, and error-context formatting by default. Reaching
/// the scalar is deliberately explicit, through [`value`](Self::value) or
/// [`to_be_bytes`](Self::to_be_bytes).
#[derive(Clone, Copy, PartialEq, Eq)]
pub struct SubjectSecret(FieldElement);
...
impl SubjectSecret {
    ...
    /// Returns the underlying canonical field element.
    ///
    /// This is the deliberate cryptographic access path; callers must not route
    /// the result into logs or diagnostics.
    #[must_use]
    pub const fn value(self) -> FieldElement {
        self.0
    }
    ...
    /// # Errors
    ///
    /// Propagates [`crate::error::ProtocolError::InvalidFieldEncoding`] for a
    /// malformed or unreduced value.
    pub fn from_decimal_str(text: &str) -> Result<Self> {
        FieldElement::from_decimal_str(text).map(Self)
    }

    /// Builds the secret from its canonical 32-byte big-endian encoding.
    ///
    /// # Errors
    ///
    /// Propagates [`crate::error::ProtocolError::InvalidFieldEncoding`] for an
    /// unreduced value.
    pub fn from_be_bytes(bytes: [u8; FIELD_BYTES]) -> Result<Self> {
        FieldElement::from_be_bytes(bytes).map(Self)
    }
...
impl fmt::Debug for SubjectSecret {
    /// Redacts the secret. The value is never rendered.
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.write_str("SubjectSecret(REDACTED)")
    }
}
...
mod tests {
    ...
    #[test]
    fn subject_secret_never_renders_its_value() {
        // Phase 0G `trade.subjectSecret`. Used only to prove it stays hidden.
        const SECRET: &str = "77112233445566778899";
        let secret = SubjectSecret::from_decimal_str(SECRET).unwrap();

        let debug = format!("{secret:?}");
        assert!(
            !debug.contains(SECRET),
            "Debug must not disclose the subject secret"
        );
        assert_eq!(debug, "SubjectSecret(REDACTED)");

        // The same redaction must hold when the secret is nested inside another
        // Debug-formatted structure, which is how it would reach a log line.
        let nested = format!("{:?}", Some((secret, 1u8)));
        assert!(
            !nested.contains(SECRET),
            "nested Debug must not disclose the subject secret"
        );

        // The scalar is still reachable through the explicit accessors, which
        // is what the commitment engine uses.
        assert_eq!(
            secret.value(),
            FieldElement::from_decimal_str(SECRET).unwrap()
        );
        assert_eq!(
            SubjectSecret::from_be_bytes(secret.to_be_bytes()).unwrap(),
            secret
        );
    }
