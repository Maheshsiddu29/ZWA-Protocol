...
Canonical ZWA Protocol types, validation, and state rules. This crate is the single source of truth for the frozen Phase 0F/0G contract: canonical `AssetBase` and raw Orchard receiver byte types, BN254 field values, 64-bit amount/nonce/expiry types, `TradeIntent`, typed protocol errors, shared signed-root envelope primitives, proof-verification interfaces, and the matcher replay state model.
